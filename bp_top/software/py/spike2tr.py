...
for i in xrange(len(lines)):
  ...
  if("ecall" in line):
    break
  elif(line[0] == "core" and line[2][:2] == "0x"):
    skip = 0
    instr_v = 1
    pc = line[2]
    instr_hex = line[3][1:-1]
    if(pc == "0x0000000080000000"):
      start = 1
    ...
    for k in xrange(len(line)):
      instr_str = instr_str + str(line[k]) + ' '
    if(instr_str[:3] == "csr"):
      skip = 0;
    ...
    if(start and (not skip)):
      recv.append(['0', regNum, wrData, pc, instr_str])
    
  elif(line[0] == "mem" and line[2] == 's'):
    if(start and (not skip)):
      if(len(line) == 4):
        recv.append(['1', line[1], line[3], pc, instr_str])
      elif(len(line) == 6):
        recv.append(['1', line[5], line[3], pc, instr_str])
      elif(len(line) == 5):
        recv.append(['1', line[4], line[1], pc, instr_str])
